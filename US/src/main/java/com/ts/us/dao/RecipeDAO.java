...
public class RecipeDAO {
	...
	public List<Recipe> getRecipes(int cuisineId, int branchId)
		...
		try {
			...
			connection = DAOUtility.getConnection();
			preparedStatement = connection
					.prepareStatement("select r.*,price,image_name from recipe r, cuisine c, serve s where r.cuisine_id = c.id  and r.id = s.recipe_id and c.id = ? and s.branch_id = ?");
			preparedStatement.setInt(1, cuisineId);
			...
			if (resultSet.next()) {
				...
				do {
					...
					recipe.setFeedbackList(new FeedbackDAO()
							.getRecipeFeedbacks(recipe.getId(), branchId));
					recipesList.add(recipe);
				} while (resultSet.next());
			}
	...
	public boolean addRecipeToBranch(long recipeId, long branchId, float price,
			String imagePath) throws UrbanspoonException {
